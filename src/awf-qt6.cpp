 ...
 *
 * Translations update:
 *  https://github.com/qt/qttranslations/blob/v6.11.3/translations/qtbase_fr.ts
	...
	if (must_save_accels) {
		...
		if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
			...
			auto parseKey = [](Qt::Key k) {
				// 0-9
				if ((k >= Qt::Key_0) && (k <= Qt::Key_9)) return QStringLiteral("KP_") + QString(QChar(int(k)));
				// a-z
				if ((k >= Qt::Key_A) && (k <= Qt::Key_Z)) return QString(QChar(int(k)).toLower());
				// simple
