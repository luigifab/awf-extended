...
#define GETTEXT_PACKAGE "awf-qt5"
...
int main(int argc, char **argv) {
	...
	if (awf_gqss) {
		...
		awf_load_themes(list_user_theme, QDir(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation)).filePath("themes"));
		awf_load_themes(list_user_theme, QDir::home().filePath(".themes"));
		list_user_theme.sort(Qt::CaseInsensitive);
	}

	// load available languages (.../locale/<lang>/LC_MESSAGES/awf-qt5.mo)
...
static void awf_load_themes(QStringList& themes, QString directory) {
	...
	if (dir.exists()) {
		...
		for (QString theme : entries) {
			if (QDir(dir.filePath(theme + "/qt5")).exists() && !themes.contains(theme))
				themes.append(theme);
		}
	}
}
...
static void on_sighup(int signum) {
	...
	#elif QT_VERSION >= QT_VERSION_CHECK(5,4,0)
		QTimer::singleShot(0, [](){ update_theme("refresh"); });
	#else
...
static void create_window() {
	...
	#elif (defined (Q_OS_WIN) || defined (_WIN32)) && (QT_VERSION >= QT_VERSION_CHECK(5,2,0))
	...
	#if defined (Q_OS_WIN) || defined (_WIN32)
		#if QT_VERSION >= QT_VERSION_CHECK(5,14,0)
			window->move(QGuiApplication::screens().first()->availableGeometry().center() - window->rect().center());
		#else
