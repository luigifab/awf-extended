#!/bin/bash
# Debian: sudo apt install dpkg-dev devscripts build-essential dh-make dh-autoreconf intltool qt6-base-dev
...
for serie in experimental resolute questing noble; do
	...
	dh_make -s -y -f ../awf-extended-$version.tar.gz -p awf-$qt

	rm -rf debian/*/*ex debian/*ex debian/*EX debian/README* debian/*doc*
	cp scripts/debian-$qt/* debian/
	cp scripts/debian-qt/*$qt* scripts/debian-qt/copyright scripts/debian-qt/metadata scripts/debian-qt/watch debian/
	head -n -1 debian/*$qt*.install > debian/install ; rm debian/awf-$qt.install
	rm -f debian/deb.sh
	...
	if [ $serie = "experimental" ]; then
		echo "=========================== lintian ($serie) =="
		lintian -EviIL +pedantic awf-${qt}_$version*.changes
		rm *amd64.changes
	elif [ $serie = "unstable" ]; then
		echo "=========================== debsign ($serie) =="
		debsign awf-$qt*$version-*_source.changes
	else
		echo "=========================== debsign ($serie) =="
		debsign awf-$qt*$version*$serie*source.changes
	fi
	cd ..
done
